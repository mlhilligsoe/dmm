...
int run(int argc, char **argv)
{
	...
	serial::Serial cSerial(port, baud, serial::Timeout::simpleTimeout(1000));
	...
	Global_Func = (char)Go_Absolute_Pos;
	long pos = 321456;
	DlgRun::Send_Package(Axis_Num, pos, cSerial);
	/* (2) Move motor 2 to absolute position of 321,456 - Method 2 - Using subroutine function*/
	DlgRun::MoveMotorToAbsolutePosition32(2, 321456, cSerial);
	...
	*/
	...
	char ID = 0; //Suppose read 0 axis motor
	Global_Func = General_Read;
	DlgRun::Send_Package(ID, Is_AbsPos32, cSerial);
	while (i<10000) //10~20ms waiting
	{
		i++;
	}
	DlgRun::ReadPackage(cSerial); //Motor absolute position stored in Motor_Pos32 variable
	...
	DlgRun::ReadMotorTorqueCurrent(cSerial); //Motor torque current stored in MotorTorqueCurrent variable
											 /* (6) Reading the main gain of 8th axis servo drive using subroutine function*/
	DlgRun::ReadMainGain(8, cSerial); //Main Gain stored in MainGain_Read variable
